...
class LicenseGenerator:
    ...
    LICENSE_PATH = (
        str(pathlib.Path(__file__).parent.absolute()) + "/licenses/{_type}.license"
    )

    @classmethod
    def has_license(cls, _type: str) -> bool:
        """Returns if the license exists
        ...
        """
        return path.exists(cls.LICENSE_PATH.format(_type=_type))

    @classmethod
    def get_format(cls, _type: str) -> str:
        ...
        """
        with open(cls.LICENSE_PATH.format(_type=_type)) as file:
            fmt = file.read()
            file.close()
            return fmt

    @classmethod
    def get_license(cls, _type: str, **kwargs) -> str:
        ...
        """
        fmt = cls.get_format(_type=_type)
    ...
    @classmethod
    def create_license_file(cls, license: str) -> bool:
        ...
        try:
            ...
            with open(cls.LICENSE_FILE_NAME, "w") as file:
                # write to the file
                file.write(license)
                # close file
                file.close()
    ...
    def create(cls, **kwargs) -> bool:
        ...
        if not cls.has_license(_type):
            print(
                f"{_type} is an unsupported license, please add the license to the 'licenses' folder"
            )
            return False

        license = cls.get_license(_type=_type, **kwargs)
